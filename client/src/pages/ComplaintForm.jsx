  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px' }}>
        <div className="glass-strong fade-in" style={{ width: '100%', maxWidth: 520, padding: '40px' }}>
          <h1 style={{ fontSize: 26, fontWeight: 800, color: '#fff', letterSpacing: '-0.5px', marginBottom: 6 }}>File a Complaint</h1>
          <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.38)', marginBottom: 32 }}>We'll look into it and get back to you</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {types.map(t => (
                <div key={t.value} onClick={() => setForm({ ...form, type: t.value })}
                  style={{ padding: '16px', borderRadius: 14, cursor: 'pointer', textAlign: 'center', transition: 'all 0.2s', border: form.type === t.value ? '1px solid rgba(255,255,255,0.35)' : '1px solid rgba(255,255,255,0.08)', background: form.type === t.value ? 'rgba(255,255,255,0.12)' : 'rgba(255,255,255,0.04)' }}>
                  <p style={{ fontSize: 22, marginBottom: 6 }}>{t.icon}</p>
                  <p style={{ fontSize: 13, fontWeight: 600, color: form.type === t.value ? '#fff' : 'rgba(255,255,255,0.5)' }}>{t.label}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {message && <p style={{ marginTop: 14, fontSize: 13, color: message.includes('✅') ? '#86efac' : '#fca5a5' }}>{message}</p>}
        </div>
      </div>
    </div>
  )
